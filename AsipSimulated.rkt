...
(define traffic-light%
  (class canvas% ; The base class is canvas%
    ...
    (define/public fill-light (lambda (dc) 
                                ; cond - pin set?
                                (send dc set-brush col (cond [ (not (zero? (hash-ref pin-state pin))) 'solid] (else 'transparent)))
...
(for-each (λ (e) (hash-set! pin-state e OFF)) (set->list (get-all-pins lights-config)))

; ASIP interface

(define digital-read (λ (pin) 
                       (hash-ref pin-state pin LOW)))

(define digital-write (λ (pin value) 
                        (hash-set! pin-state pin value) 
                        (refresh-lights)))

(define set-arduino-pin! (λ (pin) 
                           (digital-write pin ON)))

(define clear-arduino-pin! (λ (pin)
                             (digital-write pin OFF)))

(define set-pin-mode (λ (pin mode) (hash-set! pin-state pin OFF)))
...
(define open-asip (λ () (send frame show #t)))
(define close-asip (λ () null)); close the window?
